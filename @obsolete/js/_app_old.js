var dtwg = {};
//dtwg.query = jQuery.noConflict( true );
...
//dtwg.query(function() {
    ...
    var $ = dtwg.query; // local definition of jQuery
    ...
    var I18N = {
        ...
        "en": {
            "error-when-sending-data": "A problem occurred when sending the data. Please try again later.",
            "success-when-sending-data": "Your alert has been successfully sent",
            "why-i_am_forced_to": "Why do %s force you?",
            "why-i_am_angry": "Why do %s make you angry?",
            "why-i_am_wondering": "Why %s makes you ask yourself questions?"
        }
    }
    // fallback system for i18n
    console.log ("DT-WG: Browser language is: " + LANG);
    if (!I18N[LANG]) {
        var retryLANG = LANG.replace(/-.*/,'');
        console.log("DT-WG: switching from " + LANG + " to " + retryLANG);
        LANG = retryLANG;
    }
    if (!I18N[LANG]) {
        var retryLANG = "en"; // default
        console.log("DT-WG: switching from " + LANG + " to " + retryLANG);
        LANG = retryLANG;
    }
